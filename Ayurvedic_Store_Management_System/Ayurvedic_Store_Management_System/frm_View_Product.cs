...
namespace Ayurvedic_Store_Management_System
{
    public partial class frm_View_Product : Form
    {
        ...
        private void btn_Search_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand cmd = new SqlCommand("Select Product_ID,Category,Product_Name,Purchase_Price,Sales_Price,Stock,Mfg_Date,Expiry_Date,Description from Product_Details where Category = " + cmb_Category.Text + "", Con);

            var obj = cmd.ExecuteReader();

            if (obj.Read())
            {
                cmb_Category.Text = obj.GetString(obj.GetOrdinal("Category"));
